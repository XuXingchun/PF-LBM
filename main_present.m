...
se=0.6;
tau_c=3 * Mphi  + 0.5;
eta=3*Mphi/(tau_c-0.5) + 3*Mphi*se;  
gamc=1+se*(tau_c-0.5);

S= [1.0 se se 1/tau_c 1/tau_c 1/tau_c 1/tau_c 1.0 1.0];    
S=diag(S);
S(1,2)=-(4-3*eta)/(4-2*eta)*(gamc-1)/gamc * se;
S(1,3)=-(gamc-1)/gamc * se;
inv_M_S = M\S*M;
%--------------------------------------------------------------------------
filename=sprintf('presentMRT_His_U0%1.3f_M%1.4f_se%1.2f.txt',U0,Mphi,se);
fid=fopen(filename,'w+');
fprintf(fid,'t  phi_min  phi_max  Mass_C  ErrPhi \r\n');

file_pf=sprintf('presentMRT_Res_U0%1.3f_M%1.4f_se%1.2f.dat',U0,Mphi,se);
...
for j=1:NY+2
    for i=1:NX+2
        ...
        ux(i,j) = - U0 *sin(4* pi * ((i - 1.5)/Lx +0.5))...
                    *sin(4* pi * ((j - 1.5)/Lx +0.5));
        uy(i,j) = - U0 *cos(4* pi * ((i - 1.5)/Lx +0.5))...
                    *cos(4* pi * ((j - 1.5)/Lx +0.5));
    end
end
phi0=phi;ux0=ux;uy0=uy;

[heq, hlpC]=Calculate_heq_G(NX,NY,Wc, e, w, ux, uy, phi, phil, phih,eta,gamc);
h=heq;
